...
{
    QString ret = QString("crunchcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non CRC unit input in lnReqAmount, we generate the URI with CRC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::CRC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
